...
function App(){
 const[menus,setMenus]=useState(DEFAULT),[items,setItems]=useState([]),[history,setHistory]=useState([]),[view,setView]=useState('kasir'),[notice,setNotice]=useState(''),[search,setSearch]=useState(''),[admin,setAdmin]=useState(false),[editing,setEditing]=useState(null),[online,setOnline]=useState(navigator.onLine),[syncing,setSyncing]=useState(false),[recovery,setRecovery]=useState(false),[recoveryCode,setRecoveryCode]=useState(''),[restoredTx,setRestoredTx]=useState(null);
 ...
 const longStart=(m,mark)=>timers.current.set(m.id,setTimeout(()=>{mark();minus(m)},600));
 ...
 const shareExisting=async tx=>{try{await navigator.share({title:'Nota '+tx.number,text:receiptText(tx)});setItems([]);setRestoredTx(null);setNotice(`Nota ${tx.number} siap dibagikan.`)}catch(e){if(e?.name==='AbortError'){setNotice(`Nota ${tx.number} tetap di nota aktif. Tidak dibuat nomor baru.`)}else setNotice('Share tidak tersedia.')}};
 const finalize=async send=>{if(restoredTx&&send){await shareExisting(restoredTx);return}if(restoredTx&&!send){setNotice('Nota ini sudah tersimpan. Gunakan KIRIM NOTA untuk mencoba Share lagi.');return}if(!items.length)return;if(send&&!online){setNotice('KIRIM NOTA membutuhkan internet.');return}if(!confirm(send?'Simpan nota lalu buka Share?':'Simpan nota ini?'))return;const number=await nextNo(),tx={id:crypto.randomUUID(),number,createdAt:new Date().toISOString(),items:snapshotItems(items).map(i=>({...i,itemId:crypto.randomUUID()})),total,status:'PENDING_SYNC'};const next=[tx,...history];await put('history',next);setHistory(next);setItems([]);if(send){try{await navigator.share({title:'Nota '+number,text:receiptText(tx)});setNotice(`Nota ${number} siap dibagikan.`)}catch(e){if(e?.name==='AbortError'){setItems(tx.items.map(i=>({id:i.id,name:i.name,category:i.category,price:i.price,qty:i.qty})));setRestoredTx(tx);setNotice(`Nota ${number} dikembalikan ke nota aktif. Coba KIRIM NOTA lagi.`)}else setNotice(`Nota ${number} tersimpan; Share tidak tersedia.`)}}else setNotice(`Nota ${number} berhasil disimpan.`);if(online){const r=await syncPending(next,setHistory,setNotice);if(!r.ok&&r.reason)setNotice(`Nota ${number} tersimpan. ${r.reason}`)}};
 ...
 return <main>
  ...
  <section><h2>Pilih menu</h2>{cats.map(c=><div key={c}><h3>{c}</h3><div className="grid">{menus.filter(m=>m.active&&m.category===c).map(m=><MenuButton key={m.id} menu={m} qty={items.find(i=>i.id===m.id)?.qty||0} onAdd={()=>add(m)} onLongStart={mark=>longStart(m,mark)} onLongStop={()=>longStop(m)}/>)}</div></div>)}</section>
