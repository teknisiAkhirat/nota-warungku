...
test('sync row uses stable transaction id for idempotent upsert',()=>assert.equal(transactionRow(tx,'owner').id,'tx-1'));
test('item rows use stable client item ids',()=>assert.deepEqual(itemRows(tx,'owner').map(x=>x.client_item_id),['item-1','item-2']));
